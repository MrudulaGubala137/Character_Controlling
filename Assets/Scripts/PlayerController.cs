...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
       ...
       mouseInput=new Vector2(Input.GetAxis("Mouse X"),Input.GetAxis("Mouse Y")*rotationSpeed);
        transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles.x, transform.rotation.eulerAngles.y+mouseInput.x, transform.rotation.eulerAngles.z);
        //cam.transform.rotation = Quaternion.Euler(cam.transform.rotation.eulerAngles.x +(- mouseInput.y),cam. transform.rotation.eulerAngles.y, transform.rotation.eulerAngles.z);
